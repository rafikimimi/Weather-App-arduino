...
<h2 style="font-size:20px;font-family:Times New Roman;font-weight: bold; margin-left:10px;">Select Records Range</h2>
<form method="POST" style="overflow:auto;width:100%">
<div style="float:left;height:60px;width:49%;margin-top:10px">
<div style="float:left;width:20%;height:100%;">
<h2 style="font-size:22px;font-family:Times New Roman;font-weight: bold; margin-left:10px;">From<h2>
</div>
<div style="float:left;width:78%">
<input type="text" value="<?php echo date('Y/m/d H:i:s')?>" id="datetimepicker4" name="datetime"/>
</div>
</div>
...
<?php
if(isset($_POST['view']))
{$from=date("Y-m-d H:i:s  ",strtotime(sqlSafe($_POST['datetime'])));
 $to=date("Y-m-d H:i:s",strtotime(sqlSafe($_POST['datetimepicker5'])));
 $query=mysql_query("SELECT * FROM meteolologydata WHERE addedtime BETWEEN '$from' AND '$to' ") or die(mysql_error());
...
<div id="HTMLtoPDF" name="HTMLtoPDF" style="width:100%;overflow:auto; text-align:left">
		<div style=" float:left;width:100%;text-align:left;background-color:white">
<h2 style="font-size:20px;font-family:Times New Roman;font-weight: bold;">System Weather Forecasting Report</h2>
<h2 style="font-size:20px;font-family:Times New Roman;font-weight: bold;" >Date range From <?php echo "  ".$from."To"." ".$to?></h2>
</div>
	...
	$vapour=$meteorology['humidity']/ 100 * 0.6108 * exp(17.27 * $meteorology['temperature'] / ($meteorology['temperature'] + 237.3));
		  ?>
	  <tr class="tablerow">
	  <td class="tbth " style="width:20%"><?php echo date("Y-m-d H:i",strtotime($meteorology['addedtime']))?></td>
 ...
 <h2 style="font-size:20px;font-family:Times New Roman;font-weight: bold;" >Average Collection From <?php echo "  ".$from."To"." ".$to?></h2>
</div>
	  <?php
	...
	<tr class="tablerow">
	  <td class="tbth " style="width:20%"><?php echo date("Y-m-d H:i")?></td>
...
<div style=" float:left;width:100%;text-align:left">
 <h2 style="font-size:20px;font-family:Times New Roman;font-weight: bold;" >Graph for Collected Temperature against Time From <?php echo "  ".$from."To"." ".$to?></h2>
  <?php
 ...
 if(mysql_num_rows($query)>0){
	...
	while($read=mysql_fetch_array($query)){
		$arrayTemp[$count]=$read['temperature'];
		$arrayTime[$count]=date("m-d H:i",strtotime($read['addedtime']));
		$count++;
	}
  	$chart = new LineChart(700, 400);;
	$dataSet = new XYDataSet();
	for($i=0;$i<$length;$i++){
	$dataSet->addPoint(new Point($arrayTime[$i], (float)$arrayTemp[$i]));
	}
$chart->setDataSet($dataSet);
$chart->setTitle("The Graph of Temperature vs Time");
$chart->render("demo/generated/TempvsTime.png");
 } 
  ?>
  
<img src="demo/generated/TempvsTime.png" style="width:70%;"></img>
 </div>
 
 
 <div style=" float:left;width:100%;text-align:left">
 <h2 style="font-size:20px;font-family:Times New Roman;font-weight: bold;" >Graph for Collected Humidity against Time From <?php echo "  ".$from."To"." ".$to?></h2>
  <?php
 $query=mysql_query("SELECT * FROM meteolologydata WHERE addedtime>='$from' AND  addedtime<='$to' ") or die(mysql_error());
 $arrayHumi=array();
 $arrayTime=array();
 if(mysql_num_rows($query)>0){
	...
	$dataSet = new XYDataSet();
	for($i=0;$i<$length;$i++){
	$dataSet->addPoint(new Point($arrayTime[$i], (float)$arrayHumi[$i]));
	}
$chart->setDataSet($dataSet);
$chart->setTitle("The Graph of Humidity vs Time");
$chart->render("demo/generated/HumivsTime.png");
 } 
  ?>
  
<img src="demo/generated/HumivsTime.png" style="width:70%;"></img>
 </div>

 
 <div style=" float:left;width:100%;text-align:left">
 <h2 style="font-size:20px;font-family:Times New Roman;font-weight: bold;" >Graph for Collected Vapour Pressure against Dew Point From <?php echo "  ".$from."To"." ".$to?></h2>
  <?php
 $query=mysql_query("SELECT * FROM meteolologydata WHERE addedtime>='$from' AND  addedtime<='$to' ") or die(mysql_error());
 $arrayDew=array();
 $arrayVP=array();
 if(mysql_num_rows($query)>0){
	 ...
	 $count=0;
	while($read=mysql_fetch_array($query)){
		$dewpoint=pow(($read['humidity']/100),(1/8))*(112+0.9*($read['temperature']))+0.1*($read['temperature'])-112;
	    $vapour=$read['humidity']/ 100 * 0.6108 * exp(17.27 * $read['temperature'] / ($read['temperature'] + 237.3));
		
		$VP=number_format((float)$vapour, 2, '.', '');
		$DP=number_format((float)$dewpoint, 2, '.', '');
	...
	$dataSet = new XYDataSet();
	for($i=0;$i<$length;$i++){
	$dataSet->addPoint(new Point((float)$arrayDew[$i], (float)$arrayVP[$i]));
	}
$chart->setDataSet($dataSet);
$chart->setTitle("The Graph of Vapour Pressure vs Dew Point");
$chart->render("demo/generated/VPvsDP.png");
 } 
  ?>
  
<img src="demo/generated/VPvsDP.png" style="width:70%;"></img>
 </div>
...
<form method="POST">
<button type="submit" class="loginbutton" 
 name="print" onClick="PrintDoc()" id="print" style="width:15%; height:40px; margin-right:3%;
 border-radius:5px; float:right;margin-top:2%">Print</button>
<!-- <button type="submit" class="loginbutton" name="printpr"  id="printpr" style="width:15%; height:40px; margin-right:3%;
 border-radius:5px; float:right;margin-top:2%"></button>-->
  <a href="#" class="loginbutton" style="border-radius:5px; float:right;margin-top:2%;color:white;text-decoration: none;;width:15%; height:40px;margin-right:3%" onclick="HTMLtoPDF()">Save</a>
<script type="text/javascript" src="jss/jquery.js"></script>
</form>	

<script type="text/javascript" src="jss/jspdf.js"></script>
<script type="text/javascript" src="jss/pdfFromHTML.js"></script>
	 <?php
	 }else
	 {  
...
/*--This JavaScript method for Print command--*/
    ...
    function PrintDoc() {
        ...
        var toPrint = document.getElementById('HTMLtoPDF');
var x = screen.width;
        var popupWin = window.open('', '_blank', 'width=600px,height=600px,location=no,left=200px');
...
/*--This JavaScript method for Print Preview command--*/
    ...
    function PrintPreview() {
        ...
        var popupWin = window.open('', '_blank', 'width=100%,height=150,location=no,left=200px');
