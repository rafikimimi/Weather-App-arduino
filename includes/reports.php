...
<?php
if(isset($_POST['view']))
{$from=date("Y/m/d ",strtotime(sqlSafe($_POST['datetime'])));
 ...
 $query=mysql_query("SELECT * FROM meteolologydata WHERE addedtime>='$from' AND  addedtime<='$to' ") or die(mysql_error());
	 if(mysql_num_rows($query)>0){
		...
		$totalhumidy=0;
		?>
<div id="printerble" name="printerble" style="width:100%;overflow:auto; text-align:left">
		<div style=" float:left;width:100%;text-align:left;background-color:white">
<h2 style="font-size:20px;font-family:Times New Roman;font-weight: bold;">Reports</h2>
...
<button type="submit" class="loginbutton" 
 ...
 <button type="submit" class="loginbutton" 
 name="printpr" onClick="PrintPreview()" id="printpr" style="width:20%; height:40px; margin-right:3%;
 border-radius:5px; float:right;margin-top:2%">Print Preview</button>
</form>	
	 <?php
	 }else
	 {  
...
/*--This JavaScript method for Print command--*/
    ...
    function PrintDoc() {
        ...
        var toPrint = document.getElementById('printerble');
var x = screen.width;
        var popupWin = window.open('', '_blank', 'width=600,height=150,location=no,left=200px');
